...
# AUTH-2d: the review-API token-gate / public-exposure smoke steps were
# retired together with the X-Review-Token gate (smoke scripts deleted).
# Admin auth is session-only; there is no token to probe.
...
# ---------------------------------------------------------------------------
# M12.0e-6b-2: the postgres-backfill profile steps
# (_postgres_backfill_help_step / _status_step / _tests_step) were
# removed with the retirement of postgres_backfill.
# ---------------------------------------------------------------------------
...
def _resolve_steps(args: argparse.Namespace) -> List[dict]:
    ...
    # AUTH-2d: the review-exposure and review-token-gate profiles were
    # retired with the X-Review-Token gate (admin auth is session-only).
    ...
    # M12.0e-6b-2: the "postgres-backfill" profile was removed —
    # postgres_backfill.py + run_postgres_backfill.py + test_postgres_backfill.py
    # are retired (migration complete; SQLite is an unwritten source).
...
# M12.0e-6b-2: the postgres-backfill profile parsers
# (_parse_postgres_backfill_help/status/tests_output) were removed with
# the retirement of postgres_backfill.
# ---------------------------------------------------------------------------
...
def _next_actions(overall: str, records: List[dict]) -> List[str]:
    ...
    """
    # AUTH-2d: the public-exposure / token-gate rollback hints were removed
    # with the X-Review-Token smoke steps (admin auth is session-only; there
    # is no token gate to probe for public exposure).

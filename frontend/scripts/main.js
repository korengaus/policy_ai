    ...
    // — on a cold load the 4.8MB /api/brainmap resolve takes longer than 3s,
    // so the timer painted the pool fallback (a DIFFERENT claim) and the late
    // pick then swapped it: for that window the page asserted a hero under a
    // basis line describing a selection that had not happened. Ruling: timer
    // expiry keeps the SKELETON; a fallback claim paints only on an EXPLICIT
    // terminal failure of the pick (fetch rejected, !response.ok, no
    // qualifying cluster, every candidate row failed — all of which call
    // settleHeroBand() immediately). This timer is now only a HANG CEILING so
    // a request that never returns cannot hold an empty box forever: 90s,
    // above the ~80s a 4.8MB transfer needs at ~500kbps (slow-3G), so a slow
    // but PROGRESSING load still ends in the true pick with no swap. Only a
    // hang that somehow resolves after 90s can still repaint the band (the
    // late-pick pass-through in settleHeroBand is kept on purpose — the
    // pick, when it exists, always wins).
    const HERO_BAND_SETTLE_TIMEOUT_MS = 90000;

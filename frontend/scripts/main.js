    ...
    // CLUSTER-FOLD: result_id -> cluster stable_id, ONLY for rows the size map
    // already counted (>=2). Filled by one /api/cluster/{id}/members call per
    // counted row, all in parallel. A failed call or found:false leaves the id
    ...
    // feed does today. No cluster key is ever guessed client-side.
    ...
    async function loadClusterFoldKeys(ids) {
      ...
      await Promise.all(pending.map(async (id) => {
        try {
          ...
          if (!response.ok) return;
          const data = await response.json();
          const key = data && data.found === true && data.cluster
            && typeof data.cluster.stable_id === "string"
            ? data.cluster.stable_id : "";
          if (key) clusterFoldKeyMap.set(id, key);
        } catch (_) {
          // fail-silent: this id stays keyless -> unfolded (today's feed)
        }
      }));
    }

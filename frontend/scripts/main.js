    ...
    function renderEvidenceSources(sources, cardHasGenuineOfficial = true) {
        ...
        <div class="source-list">
          ${list.map((source) => {
            ...
            const reason = escapeHtml(clampAuthority
              ...
              : userFacingReportText(source.reliability_reason || "", ""));
    ...
    // unchanged.
    function renderSourceCandidates(sourceCandidates, cardHasGenuineOfficial = true) {
    ...
    async function resolveTrendingHeroPick() {
      ...
      for (const row of rows) {
        ...
        // repaint-when-landed (the renderTrendingTop5 posture): the home
        // render is idempotent, so this just upgrades the hero in place.
        renderHotTopics();
        return;
      }
    }
    ...
    function renderHotTopics(preferredResults) {
      ...
      if (dailyPick) {
        // POLISH-LABELS 1: surface WHY this card is the hero — the trending
        // row's outlets + growth, in the sidebar Top-5 format VERBATIM
        // ("{N}개 매체 · ↑{g}" / " · NEW"; badge omitted when growth is
        // absent/zero/non-finite — never "↑0"). BOTH numbers come from the
        // SAME /api/trending row (snapshot axis); the card face's cluster-size
        // chip is the live-graph axis and stays unpaired with the delta.
        ...
        hotTopicsTopEl.innerHTML =
          `<div class="public-eyebrow">확산 성장${heroStat ? ` · ${heroStat}` : ""} · 주간 스냅샷 기준 · 검증이 아닙니다</div>`
        ...
        // POLISH-LABELS 2: Branch B now states its basis. The two-card hero is
        // ranked by sortTopicCards(filtered, "뜨는순") — severity + freshness +
        // 근거 수준 — whose public label is the existing sort-dropdown option
        // "주목순" (SORT-LABEL-HONESTY, template.html); the tail reuses the
        // Branch A eyebrow's honesty fragment. The eyebrow is a SIBLING placed
        // before .feed-hero-band (exactly the Branch A structure), so the
        // band's grid columns / divider rule / <=960px stacking are untouched.
        // Also correct on the 전체 tab's trending-failure fallback — the
        // ordering there is the same 뜨는순.
        const band = `<div class="feed-hero-band">`
          ...
          + `</div>`;
        hotTopicsTopEl.innerHTML =
          '<div class="public-eyebrow">주목순 · 검증이 아닙니다</div>' + band;
        hotTopicsEl.innerHTML = pageSlice
          .map((card) => renderTopicCardHtml(card, { detailed: true }))
          .join("");
      } else {
        // <2 fallback — a single card renders as the hero alone (no grid below).
        // POLISH-LABELS 2: same basis, same label as the two-card branch.
        hotTopicsTopEl.innerHTML =
    ...
    function getResultPipelineParts(result) {
      ...
      const cardHasGenuineOfficial = (typeof sourceReliabilitySummary.has_genuine_official_support === "boolean")
        ? sourceReliabilitySummary.has_genuine_official_support
        : (numberValue(debugSummary.official_body_matches, 0) > 0);
    ...
    async function loadSpreadAnnotations() {
      ...
      for (const section of placeholders) {
        ...
        try {
          ...
          // cluster's earliest member even predates the 07-14 decision). MUST
          // stay in lockstep with the share-image canvas line above (two render
          // sites, one string).
          const timelineLine = Number(timeline.dated_members) > 0 && firstAt && lastAt
            ? `<div class="spread-timeline-line">확산 기간 ${escapeHtml(firstAt)} → ${escapeHtml(lastAt)} (수집 기사 기준)</div>`
            : "";
    ...
    function renderResults(results, focusIndex = selectedResultIndex) {
      ...
      resultsEl.innerHTML = displayResults.map((result) => {
        ...
        const verificationDetails = [
          ...
          renderCollapsibleSection(
            "근거 요약과 부족한 맥락",
            `${renderEvidenceExtractionSummary(evidenceExtractionSummary)}${renderEvidenceSources(verification.evidence_sources, cardHasGenuineOfficial)}${advDefList([

    ...
    async function loadClusterFoldKeys(ids) {
      ...
      try {
        ...
        if (data && typeof data.graph_generated_at === "string" && data.graph_generated_at) {
          ...
          // 113-APPLY: the date can land after the grid's first paint —
          // refresh the grid-level basis line without waiting for a repaint.
          updateFeedBuildBasis();
        }
    ...
    // HONEST-FAILURE (96-APPLY): true when the ONE home-feed /history fetch
    // (init-time, 전체) failed — the feed's empty grid then shows the shipped
    // STABLE-TABS failure line instead of the empty-state instruction, which
    // told a reader to run a search rather than that loading failed.
    let homeFeedFetchFailed = false;
    ...
    function stripInternalDiagnosticText(value) {
      ...
      const lineReplacements = [
        ...
        // (snake-case zero class). Bounded to the byline shape — 기자/특파원
        // followed by an @-address — so an informational email inside real
        // prose can never be eaten; the role word itself stays.
        [/(기자|특파원)\s*[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]*/g, "$1"],
    ...
    // ESCAPED-MARKUP (101-APPLY): stored evidence text from the FSS lane
    // (2026-06-18 → 08-21, 294 rows) carries literal "<p> <span style=…>"
    // because the provider decoded entities AFTER stripping tags; the
    // producer is fixed, stored rows are not rewritten, so the reader-text
    // launder drops real tag shapes (ASCII tag name after "<") here. Korean
    // angle-bracket quotes ("<가계부채>") are not tags and stay. (Regex kept
    // inline: the render scan extracts this function verbatim.)
    function userFacingReportText(value, fallback = "-") {
      const text = stripInternalDiagnosticText(
        typeof value === "string" ? value.replace(/<\/?[A-Za-z][^<>]*>/g, " ").replace(/\s{2,}/g, " ").trim() : value);
    ...
    // BUILD-BASIS (113-APPLY): ONE grid-level basis line for the feed's
    // 매체-N chips (and the region-1 기사-건수 figures) — a note under every
    // chip would drown the cards, and every chip on the grid comes from the
    // SAME newest brainmap_graph row (cluster-sizes and cluster-spreads both
    // serve the same server-cached row; a mix would need a rebuild landing
    // inside one page-load's fetch window). The date is the additive
    // graph_generated_at captured from the SAME cluster-spreads batch the
    // grid's circulation facts come from; unknown -> no line, never a guess.
    // Sits beside the sort-basis eyebrow in the feed toolbar and is NOT
    // gated on the active sort — chips render under every ordering.
    function updateFeedBuildBasis() {
      const sortBasisEl = document.getElementById("feedSortBasis");
      if (!sortBasisEl || typeof clusterSpreadGraphGeneratedAt !== "string"
        || !clusterSpreadGraphGeneratedAt) return;
      let node = document.getElementById("feedBuildBasis");
      if (!node) {
        node = document.createElement("span");
        node.id = "feedBuildBasis";
        node.className = "public-eyebrow";
        sortBasisEl.insertAdjacentElement("afterend", node);
      }
      node.textContent = `기준 시점: ${clusterSpreadGraphGeneratedAt.slice(0, 10)} 브레인맵`;
    }
    ...
    function renderHotTopics(preferredResults) {
      ...
      if (!hot.length) {
        ...
        } else if (homeFeedFetchFailed && isHomeFeedRender && activeDomain === "전체") {
          // HONEST-FAILURE (96-APPLY): the init /history fetch failed. Reuse
          // the STABLE-TABS failure line verbatim (전체 is the active row of
          // the same 분야 tab strip) — a failure must never render the
          // empty-state instruction below, which asserts the corpus works.
          writeFeedGridHtml('<div class="empty-state">이 분야의 뉴스를 불러오지 못했습니다. 잠시 후 다시 시도해 주세요.</div>');
        } else {
    ...
    function officialPeriodicEditionMismatch(result) {
      ...
      try {
        ...
        candidates.forEach((cand) => {
          ...
          // PERIOD-SIBLING (101-APPLY): the period test is taken from the
          // periodic-edition titles THEMSELVES. Before, every primary official
          // title contributed its periods, so a year-only sibling ("2026년
          // 상반기 지역별고용조사 …" → (2026, ∅)) agreed with any month of that
          // year and let a 2026년 5월 claim keep its "26년 7월 고용동향" as
          // confirmed (id 16105). Measured over all 1,130 body-match rows:
          // exactly one row changes (16105).
          if (!OFFICIAL_PERIODIC_FAMILY_RE.test(title)) return;
          docAllPeriods.push(...periods);
          hasPeriodicEdition = true;
        });
    ...
    function publicSourceCards(result) {
      ...
      // EVIDENCE-FIRST (96-APPLY): the same official document is stored TWICE —
      // full strength fields on its source_candidates copy (which line
      // "origin === candidate" keeps out of the public rows) and stripped
      // fields on its evidence_sources copy (which therefore under-badges as
      // 공식 약한 후보). Record 16000's 산림청 direct match rendered 공식
      // 직접 근거 inside the collapse and 약한 후보 on the visible row — one
      // document, two claims. Thread the candidate twin's OWN stored fields
      // (classification / body_match / score, matched by url else title) onto
      // the public copy so sourceTraceability's EXISTING tiers read the same
      // data everywhere. Gated by the shipped genuine predicate: on a card
      // without genuine official support nothing is enriched, so the honest
      // top line can never be out-claimed (OFFICIAL-EVIDENCE-DISPLAY-HONESTY
      // posture) and such cards render byte-identically.
      const cardDebug = (result?.verification_card || result || {}).debug_summary || result?.debug_summary || {};
      if (officialEvidenceIsGenuine(reliability, cardDebug)) {
        const strengthByKey = new Map();
        candidates.forEach((candidate) => {
          if (!candidate) return;
          const cls = candidate.official_evidence_classification
            || candidate.official_direct_match_classification;
          if (!cls) return;
          [candidate.url || candidate.source_url || "",
           candidate.title || candidate.source_title || ""].forEach((key) => {
            if (key && !strengthByKey.has(key)) strengthByKey.set(key, candidate);
          });
        });
        merged.forEach((row) => {
          if (row.source_origin === "candidate") return;
          if (row.official_evidence_classification || row.official_direct_match_classification) return;
          const twin = strengthByKey.get(row.url || row.source_url || "")
            || strengthByKey.get(row.title || row.source_title || "");
          if (!twin) return;
          row.official_evidence_classification = twin.official_evidence_classification
            || twin.official_direct_match_classification;
          row.official_body_match = twin.official_body_match;
          row.official_evidence_score = twin.official_evidence_score
            ?? twin.official_final_direct_match_score;
        });
      }
      const seen = new Set();
      ...
      // EVIDENCE-FIRST (96-APPLY): the visible three were simply the FIRST
      // three survivors of the merge order (evidence_sources → snippets), so
      // a card's strongest official match could lose its slot to weaker rows
      // that happened to merge earlier — record 16000 claimed direct support
      // while its 공식 직접 근거 row sat below the cut. Stable-sort by the
      // SAME tier the row's own badge states (sourceTraceability — direct,
      // then context, then the rest, original order preserved within a tier)
      // and cut afterwards. Nothing a row says about itself changes, weak
      // candidates are reordered rather than suppressed, and cards with no
      // official tier sort entirely stable (byte-identical render).
      const traceTierRank = (source) => {
        const label = sourceTraceability(source, reliability).label;
        if (label === "공식 직접 근거") return 0;
        if (label === "공식 맥락 근거") return 1;
        return 2;
      };
      const ranked = unique
        .map((source, index) => [traceTierRank(source), index, source])
        .sort((a, b) => a[0] - b[0] || a[1] - b[1])
        .map((entry) => entry[2])
        .slice(0, 3);
      return { items: ranked, reliability, officialLimitation: publicOfficialLimitationNote(result, reliability, ranked) };
    }
    ...
    // BUILD-BASIS (113-APPLY): optional basisNote — the distribution is
    // computed per graph row (build-derived), so the caller may append the
    // build's 기준 시점 line under the caption; "" keeps today's markup.
    function corpusDistributionHtml(corpus, outletCount, basisNote) {
    ...
    async function loadSpreadAnnotations() {
      ...
      for (const section of placeholders) {
        ...
        try {
          ...
          const spreadFactCell = (label, value, basis) => `<div class="spread-fact"><span class="spread-fact-label">${label}</span><span class="spread-fact-value">${value}</span>${basis || ""}</div>`;
          // BUILD-BASIS (113-APPLY): the grid mixes bases — 매체 수 / 보도
          // 건수 / 문구 거의 동일 are figures of the graph BUILD, 확산 기간 is
          // a per-request read already covered by the grid-level collected-
          // basis note directly below. So the build cells carry their OWN
          // in-cell 기준 시점: <date> 브레인맵 (weekly.html's shipped
          // construction; the hero cells' 112-APPLY discipline), dated from
          // THIS response's additive graph_generated_at — the same payload
          // the counts came in, so the timestamp can never describe data it
          // did not arrive with. Absent field (old graph row / older server)
          // -> no note, never a guess.
          const spreadBuildDay = (typeof data?.graph_generated_at === "string" && data.graph_generated_at)
            ? data.graph_generated_at.slice(0, 10) : "";
          const spreadBuildBasisNote = spreadBuildDay
            ? `<span class="spread-facts-note spread-fact-basis">기준 시점: ${escapeHtml(spreadBuildDay)} 브레인맵</span>`
            : "";
          // POLISH-LABELS 4: corpus-scoped wording (claim.html's solved form) —
          // first_at is the earliest COLLECTED member, not the claim's origin
          ...
          const nearCell = nearShown === null
            ? ""
            : spreadFactCell("문구 거의 동일", `${escapeHtml(formatCount(nearShown))}<span class="fact-unit">개 매체</span>`, spreadBuildBasisNote);
          ...
          const sizeCell = Number.isFinite(memberSize) && memberSize >= outletCount
            ? spreadFactCell("보도 건수", `${escapeHtml(formatCount(memberSize))}<span class="fact-unit">건</span>`, spreadBuildBasisNote)
            : "";
          ...
          const factsGrid = `<div class="spread-facts">${
            spreadFactCell("전체 확산 매체 수", `${escapeHtml(formatCount(outletCount))}<span class="fact-unit">개 매체</span>`, spreadBuildBasisNote)
          ...
          const chartRow = (() => {
            // BUILD-BASIS (113-APPLY): the distribution is build-derived, so
            // its 기준 시점 line goes INSIDE its own block — a row-level line
            // would also appear to cover the per-request daily strip beside it.
            const distHtml = corpusDistributionHtml(data?.corpus_outlet_distribution, outletCount,
              spreadBuildDay ? `<div class="spread-facts-note spread-fact-basis">기준 시점: ${escapeHtml(spreadBuildDay)} 브레인맵</div>` : "");
    ...
    function truncateCardFaceClaim(text, maxLength) {
      ...
      // QUOTE-CLOSE (99-APPLY): a budget cut can stop IMMEDIATELY BEFORE a
      // closing curly quote (id 15912: the claim carries "“내가 방에 왜
      // 왔지?”" across the 115 boundary, so the face ended …왔지? with the ”
      // one character away) — the face then shows an opened quote that never
      // closes, mid-quotation. A new input shape (curly-quoted headline runs
      // glued into claim text), not a bypassed path: no earlier trim pass
      // looks at quotes. When the very next character of the source is the
      // closing mark AND the cut holds its unclosed opener, append that one
      // mark — the quotation completes, nothing is dropped, and every other
      // face is returned byte-identical. (A first attempt backed the whole
      // quoted fragment off instead; over the live corpus that re-exposed
      // binding tails on 29 faces, so it was replaced with this.) Inline —
      // the render audit pins main.js helpers by name.
      if (value.startsWith(cut)) {
        const next = value.charAt(cut.length);
        const opener = next === "”" ? "“" : (next === "’" ? "‘" : "");
        if (opener) {
          const lastOpen = cut.lastIndexOf(opener);
          if (lastOpen !== -1 && cut.indexOf(next, lastOpen + 1) === -1) {
            cut += next;
          }
        }
      }
    ...
    // HONEST-FAILURE (96-APPLY): failure returns null (the STABLE-TABS domain
    // fetch contract) instead of [] — an empty array said "the corpus is
    // empty" for a dropped connection, and the feed then rendered the
    // empty-state instruction instead of a failure line.
    ...
    async function searchFirst() {
      ...
      // pre-total behavior.
      let searchTotal = 0;
      // HONEST-FAILURE (96-APPLY): a rejected/failed fetch used to fall into
      // the SAME branch as a genuine zero-hit result, so a dropped connection
      // rendered "기존 분석이 없습니다" — a false claim about the corpus.
      // Failure and miss are now separate states. NOTE the residual: the
      // server's own exception handler answers 200 with the empty payload
      // (api_server.py search_corpus → _empty()), and that case is
      // indistinguishable from a real miss on this side.
      ...
      if (hits.length) {
        ...
        // No search-scoped failure sentence ships anywhere; the shipped RETRY
        // sentence (the second sentence of the STABLE-TABS feed line, the map
        // status line and the login error) is reused verbatim, asserting
        // nothing about the corpus. The progress line is hidden rather than
        // given a claim it cannot support.
        v2SetProgressVisible(false);
        renderSearchFailure();
      } else {
        // The offer clause was a promise only the operator's view keeps —
        // anonymous readers were told a proposal was coming that never
        // rendered (ANALYZE-EXPOSURE removed it). Same string for the
        // operator; its shipped first clause alone for everyone else.
        v2UpdateProgress(100, operatorToolsFlagSet()
          ? "기존 분석 없음 — 새 분석을 제안합니다"
          : "기존 분석 없음");
    ...
    (async () => {
      ...
      // HONEST-FAILURE (96-APPLY): null = the fetch FAILED (network/HTTP) —
      // render the failure line, never the empty-state instruction.
      homeFeedFetchFailed = serverResults === null;

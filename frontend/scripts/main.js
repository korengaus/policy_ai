    ...
    // DOMAIN-HERO-MIN (86-APPLY): a domain tab's fallback band renders only
    // when its top cluster has spread to at least this many outlets. Measured
    // live (2026-08-20): all 13 domain tabs rendered a hero, 11 of them from a
    // top count of 2-4 — the same visual weight the whole-record hero carries
    // at 80 outlets, so size implied a significance the data does not hold.
    // A circulation rule, not a content judgement: every record stays
    // eligible and simply has to earn the slot. Below the line the tab is
    // bandless (the existing HERO-NO-JUDGEMENT state — rows begin directly,
    // nothing leaves the grid). 전체 is exempt on every branch: the
    // whole-record pick and the home pool fallback are unchanged.
    ...
    function renderHotTopics(preferredResults) {
      ...
      // DOMAIN-HERO-MIN (86-APPLY): on a domain tab the band must EARN its
      // slot — hot[0] (the pool's top by whole-record outlet count; absent
      // from clusterSizeMap reads 0) needs >= DOMAIN_HERO_MIN_OUTLETS. This
      // ONE flag gates the band markup (the `circulation` arg below), the
      // grid hold-back (heroKeys) and the hero-alone fallback together, so a
      // suppressed band can never orphan a card out of the grid. On 전체 it
      // equals heroCirculation exactly — home renders byte-identically
      // (paintHeroBandOnly, 전체-only by its own guard, is untouched for the
      // same reason).
      const heroBandEligible = heroCirculation && (activeDomain === "전체"
      ...
      const heroKeys = heroBandEligible
      ...
      if (heroPending) {
        ...
        hotTopicsTopEl.classList.remove("hero-band-reserved");
        writeHeroBandHtml(heroBandHtml(hot, dailyPick, heroBandEligible, cumulativePick));
      }
      if (!hot.length) {
      ...
      } else if (dailyPick || cumulativePick || heroPending || !heroBandEligible || hot.length >= 2) {

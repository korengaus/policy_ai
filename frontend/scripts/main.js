    ...
    function renderHotTopics(preferredResults) {
      ...
      // Pages over the post-hero pool. Self-CLAMP currentPage: an async /history
      // re-render can shrink the pool while a page is retained (e.g. after a detail
      // BACK), so heal a now-out-of-range page HERE rather than at every caller.
      const totalPages = Math.max(1, Math.ceil(gridPool.length / PAGE_SIZE));
      currentPage = Math.min(Math.max(1, currentPage), totalPages);
      const pageSlice = gridPool.slice((currentPage - 1) * PAGE_SIZE, currentPage * PAGE_SIZE);
      ...
      // Empty when totalPages <= 1 (small pool → no nav shown).
      renderFeedPagination(totalPages, currentPage);
    ...
    if (feedPaginationEl) {
      feedPaginationEl.addEventListener("click", (event) => {
        ...
        renderHotTopics();
        if (hotTopicsEl) hotTopicsEl.scrollIntoView({ behavior: "smooth", block: "start" });
      });
    }

    ...
    const LOCAL_HISTORY_KEY = "policy_ai_recent_analysis";
          ...
          ${list.map((source) => {
            ...
            return `
              <div class="evidence-source">
                ...
                <div class="evidence-source-meta">출처 유형: ${sourceType} · 신뢰도: ${score}/5</div>
    ...
    function renderSourceReliabilitySummary(summary) {
      ...
      const topHtml = topUrl
        ? `<a href="${escapeHtml(topUrl)}" target="_blank" rel="noopener noreferrer">${escapeHtml(topTitle)}</a>`
        : escapeHtml(topTitle);
      return `
        <div class="source-reliability-summary">
          <div class="source-reliability-tile">
            <span class="label">최고 신뢰 출처</span><br>${topHtml}
          </div>
          <div class="source-reliability-tile">
            <span class="label">공식 출처 후보 수</span><br>${escapeHtml(data.official_candidate_count ?? 0)}
          </div>
          <div class="source-reliability-tile">
            <span class="label">본문 확보 출처</span><br>${escapeHtml(data.raw_text_available_count ?? 0)}
          </div>
          <div class="source-reliability-tile">
            <span class="label">평균 출처 신뢰도</span><br>${escapeHtml(data.average_reliability_score ?? 0)}
          </div>
        </div>
        ...
        return `
          <div class="evidence-snippet">
            ...
            ${related.length ? related.map((snippet) => {
              const sourceTitle = escapeHtml(userFacingReportText(snippet.source_title || snippet.source_url || "출처", "출처"));
              const sourceUrl = escapeHtml(snippet.source_url || "");
              ...
              return `
                ...
                <div class="evidence-snippet-grid">
                  ...
                  <div><span class="label">발행처</span><br>${escapeHtml(snippet.publisher || "-")}</div>
    ...
    function renderSourceCandidates(sourceCandidates) {
      const list = Array.isArray(sourceCandidates) ? sourceCandidates.slice(0, 8) : [];
      if (!list.length) {
        return '<div class="evidence-source-meta">표시할 출처 탐색 후보가 없습니다.</div>';
      }

      return `<div class="source-candidate-list">${list.map((source) => {
        ...
        return `
          <div class="source-candidate">
            ...
            ${exclusionLabel ? `<div class="evidence-source-meta">${escapeHtml(exclusionLabel)}</div>` : ""}
            <div class="source-candidate-grid">
              <div><span class="label">검색어</span><br>${escapeHtml(source.query_used || "-")}</div>
              <div><span class="label">목적</span><br>${escapeHtml(formatSourcePurpose(source.purpose))}</div>
              <div><span class="label">출처 유형</span><br>${escapeHtml(formatSourceType(source.source_type))}</div>
              <div><span class="label">수집 방식</span><br>${escapeHtml(formatDiagnosticText(source.retrieval_method || "-"))}</div>
              <div><span class="label">발행처</span><br>${escapeHtml(publicInstitutionName(source.publisher || "-"))}</div>
              <div><span class="label">본문 확보</span><br>${source.raw_text_available ? "예" : "아니오"}</div>
              <div><span class="label">공식 본문 수집</span><br>${source.official_body_fetched ? "예" : "아니오"}</div>
              <div><span class="label">공식 본문 길이</span><br>${escapeHtml(source.official_body_length ?? "-")}</div>
              <div><span class="label">공식 본문 매칭</span><br>${source.official_body_match ? "예" : "아니오"}</div>
              <div><span class="label">공식 매칭 점수</span><br>${escapeHtml(source.official_body_match_score ?? "-")}</div>
              <div><span class="label">공식 직접 매칭</span><br>${escapeHtml(officialDirectMatchLabel(source))}</div>
              <div><span class="label">공식 직접 점수</span><br>${escapeHtml(source.official_final_direct_match_score ?? source.official_body_match_score ?? "-")}</div>
              <div><span class="label">URL 해소 점수</span><br>${escapeHtml(source.official_url_score ?? "-")}</div>
              <div><span class="label">의미 매칭</span><br>${escapeHtml(source.semantic_match_score ?? "-")}</div>
              ...
              <div><span class="label">공개 표시 판단</span><br>${escapeHtml(userFacingReportText(trace.explanation, "추가 확인 필요"))}</div>
            </div>
            ${Array.isArray(source.official_matched_sentences) && source.official_matched_sentences.length ? `
              <div class="evidence-source-meta"><strong>공식 문서 매칭 문장</strong></div>
              <ul class="compact-list">
                ${source.official_matched_sentences.slice(0, 2).map((match) => `
                  <li>${escapeHtml(match.sentence || "-")} <span class="label">(${escapeHtml(match.score ?? "-")}점)</span></li>
                `).join("")}
              </ul>
            ` : ""}
            <div class="evidence-source-meta">${escapeHtml(userFacingReportText(source.reliability_reason || "", ""))}</div>
            <div class="risk-flags">${escapeHtml(formatDiagnosticText(formatList(source.source_risk_flags)))}</div>
          </div>
        `;
      }).join("")}</div>`;
    }
    ...
    function topSummaryLine(result) {
      ...
      const line = userFacingReportText(decision.decision_summary || verification.evidence_summary || exportClaimText(result) || "", "");
    ...
    function currentTopicCards(preferredResults) {
      ...
      return topicCardsFromHistory(safeReadLocalHistory());
    }

    function renderHotTopics(preferredResults) {
      if (!hotTopicsEl) return;
      const cards = currentTopicCards(preferredResults).filter((card) => {
        return activeCategory === "전체" || card.category === activeCategory;
      });
      if (!cards.length) {
        hotTopicsEl.innerHTML = '<div class="empty-state">선택한 카테고리에 표시할 검증 뉴스가 없습니다. 검색을 실행하거나 최근 분석을 불러와 주세요.</div>';
        return;
      }
      hotTopicsEl.innerHTML = cards.map((card) => {
    ...
    function buildOfficialEvidenceNarrative(officialEvidenceState) {
      ...
      const copy = {
        ...
        candidate_only: {
          summary: state.officialEvidenceLabel || "공식 후보만 있음",
            ...
            ${items.map((source) => {
              ...
              return `
                <article class="public-source-card">
                  ...
                  <div class="public-source-meta">근거 강도: ${escapeHtml(formatTechnicalLabel(source.evidence_strength || source.evidence_quality_label || source.supports_claim || "-"))}</div>
                  <div class="public-source-meta">신뢰/관련 점수: ${escapeHtml(score)}</div>
                  <div class="public-source-meta">링크: ${url ? `<a href="${escapeHtml(url)}" target="_blank" rel="noopener noreferrer">원문 보기</a>` : "URL 없음"}</div>
        ...
        <section class="reading-guide">
          ...
          <div class="reading-guide-grid">
            ...
            <div class="reading-guide-card">
              ...
              ${escapeHtml(confidenceScore)}점은 공식 자료, 기사 근거, 반박 신호를 함께 본 참고 점수입니다.
            </div>
            <div class="reading-guide-card">
    ...
    function reviewerOfficialStatus(reliability, debug) {
      ...
      if (state.officialEvidenceStatus === "candidate_only") return "공식 후보만 있음";
      return "공식 본문 부족";
    }
    ...
    function renderResults(results, focusIndex = selectedResultIndex) {
      if (!results.length) {
        ...
        resultsEl.innerHTML = '<div class="empty-state">관심 있는 이슈의 상세 보기를 누르거나 검색어를 입력하면 검증 리포트가 표시됩니다.</div>';
        renderHotTopics();
        return;
      }
      ...
      resultsEl.innerHTML = displayResults.map((result) => {
        ...
        const url = escapeHtml(result.original_url || "#");
        const topic = exportTopicLabel(result, currentReportContext?.query || queryInput?.value || "");
        ...
        const topSourceHtml = topSourceUrl
          ? `<a href="${escapeHtml(topSourceUrl)}" target="_blank" rel="noopener noreferrer">${escapeHtml(userFacingReportText(publicInstitutionName(topSourceTitle), "확인 필요"))}</a>`
    ...
    function renderResultsLegacy(results) {
      if (!results.length) {
        metricsEl.style.display = "none";
        clearCurrentReportContext();
        resultsEl.innerHTML = '<div class="empty-state">분석을 실행하면 결과가 여기에 표시됩니다.</div>';
        return;
      }
      ...
      resultsEl.innerHTML = results.map((result) => {
        ...
        const decision = result.final_decision || {};
        const verification = result.verification_card || result || {};
        const claims = verification.claims || result.claims || [];
        const normalizedClaims = verification.normalized_claims || result.normalized_claims || [];
        const sourceQueries = verification.source_queries || result.source_queries || [];
        const sourceCandidates = verification.source_candidates || result.source_candidates || [];
        const sourceReliabilitySummary = verification.source_reliability_summary || {};
        const evidenceSnippets = verification.evidence_snippets || result.evidence_snippets || [];
        const evidenceExtractionSummary = verification.evidence_extraction_summary || {};
        const contradictionChecks = verification.contradiction_checks || result.contradiction_checks || [];
        const contradictionSummary = verification.contradiction_summary || result.contradiction_summary || {};
        const biasFramingAnalysis = verification.bias_framing_analysis || result.bias_framing_analysis || [];
        const biasFramingSummary = verification.bias_framing_summary || result.bias_framing_summary || {};
        const debugSummary = verification.debug_summary || result.debug_summary || {};
        const level = String(decision.policy_alert_level || "WATCH").toUpperCase();
        ...
        const topSourceUrl = sourceReliabilitySummary.top_source_url
          ...
          || "";
        const topSourceHtml = topSourceUrl
          ? `<a href="${escapeHtml(topSourceUrl)}" target="_blank" rel="noopener noreferrer">${escapeHtml(userFacingReportText(topSourceTitle, "확인 필요"))}</a>`
          : escapeHtml(userFacingReportText(topSourceTitle, "확인 필요"));
        const recommendedAction = sourceReliabilitySummary.official_mismatch
          ? "추가 공식 출처 확인 필요"
          : formatRecommendation(decision.action_recommendation);
        const finalScore = decision.final_score ?? confidence.policy_confidence_score ?? "-";
        const sourceTrustScore = decision.source_trust_score ?? debugSummary.source_trust_score ?? sourceReliabilitySummary.average_reliability_score ?? "-";
        const userContext = {
          level,
        ...
        const keyTakeaways = renderKeyTakeaways(userContext);
        const userSummarySections = renderUserSummarySections(userContext);
        const verificationDetails = [
          renderCollapsibleSection(
            "정규화된 주장",
            `${renderClaimList(claims)}${renderNormalizedClaims(normalizedClaims)}`
          ...
          renderCollapsibleSection(
            "근거 요약",
            `${renderEvidenceExtractionSummary(evidenceExtractionSummary)}${renderEvidenceSources(verification.evidence_sources)}<div class="evidence-source-meta">${escapeHtml(userFacingReportText(verification.evidence_summary, "-"))}</div>`
          ),
          renderCollapsibleSection(
            "부족한 맥락",
            `<div>${escapeHtml(buildSafeMissingContext(result))}</div><div class="evidence-source-meta">마지막 확인 시간: ${escapeHtml(verification.last_checked_at || "-")}</div>`
          ),
        ].join("");
        ...
        return `
          <article class="result-card">
            <div class="card-top">
              <span class="badge ${alertClass(level)}">${escapeHtml(formatAlert(level))}</span>
              <span class="badge topic-badge">${escapeHtml(result.topic || "미분류")}</span>
            </div>
            <h2 class="result-title">
              <a href="${url}" target="_blank" rel="noopener noreferrer">${title}</a>
            </h2>
            <div class="result-summary-grid">
              <div class="summary-tile">
                <span class="label">신뢰도 점수</span>
                <strong>${escapeHtml(confidence.policy_confidence_score ?? "-")}</strong>
              </div>
              <div class="summary-tile">
                <span class="label">영향도 / 위험도</span>
                <strong>${escapeHtml(formatLevel(impact.impact_level))} / ${escapeHtml(formatLevel(confidence.risk_level))}</strong>
              </div>
              <div class="summary-tile">
                <span class="label">최고 신뢰 출처</span>
                <strong>${topSourceHtml}</strong>
              ...
              <div class="summary-tile">
                <span class="label">근거 품질</span>
                <strong>strong ${escapeHtml(quality.strong ?? 0)}, medium ${escapeHtml(quality.medium ?? 0)}, weak ${escapeHtml(quality.weak ?? 0)}, avg ${escapeHtml(quality.average_evidence_quality_score ?? 0)}</strong>
              </div>
              <div class="summary-tile">
                <span class="label">근거 강도</span>
                <strong>strong ${escapeHtml(strength.strong ?? 0)}, medium ${escapeHtml(strength.medium ?? 0)}, weak ${escapeHtml(strength.weak ?? 0)}</strong>
              </div>
              <div class="summary-tile">
                <span class="label">권장 조치</span>
                <strong>${escapeHtml(recommendedAction)}</strong>
            ...
            <div class="decision-summary">${escapeHtml(decision.decision_summary)}</div>
            <section class="verification-card">
              <h3>검증 카드</h3>
              <div class="verification-intro">
                <div class="summary-tile">
              ...
              ${verificationDetails}
              <div class="verification-grid legacy-expanded">
                <div class="verification-item full">
                  <span class="label">핵심 주장</span><br>
                  ${escapeHtml(exportClaimText(result))}
                </div>
                <div class="verification-item full">
                  <span class="label">핵심 주장 목록</span>
                  ${renderClaimList(claims)}
                </div>
                <div class="verification-item full">
                  <span class="label">정규화된 주장</span>
                  ${renderNormalizedClaims(normalizedClaims)}
                </div>
                <div class="verification-item full">
                  <span class="label">근거 문장</span>
                  ${renderEvidenceSnippets(claims, evidenceSnippets)}
                </div>
                <div class="verification-item full">
                  <span class="label">반박/모순 검사</span>
                  ${renderContradictionChecks(claims, contradictionChecks)}
                </div>
                <div class="verification-item full">
                  <span class="label">프레이밍/편향 검사</span>
                  ${renderBiasFramingAnalysis(claims, biasFramingAnalysis)}
                </div>
                <div class="verification-item full">
                  <span class="label">출처 탐색 후보</span>
                  ${renderSourceCandidates(sourceCandidates)}
                </div>
                <div class="verification-item full">
                  <span class="label">출처 검색 쿼리</span>
                  ${renderSourceQueries(sourceQueries)}
                </div>
                <div class="verification-item">
                  <span class="label">AI 초안 판정</span><br>
                  <strong>${escapeHtml(safeAiDraftVerdictForExport(result))}</strong>
                </div>
                <div class="verification-item">
                  <span class="label">신뢰도</span><br>
                  <strong>${escapeHtml(verification.verdict_confidence ?? "-")}</strong>
                </div>
                <div class="verification-item">
                  <span class="label">출처 신뢰도</span><br>
                  ${escapeHtml(verification.source_reliability_score ?? "-")}/5
                  <div class="evidence-source-meta">${escapeHtml(userFacingReportText(verification.source_reliability_reason || "", ""))}</div>
                </div>
                <div class="verification-item">
                  <span class="label">리뷰 상태</span><br>
                  ${escapeHtml(formatReviewStatus(verification.review_status))}
                </div>
                <div class="verification-item full">
                  <span class="label">근거 출처</span>
                  ${renderEvidenceSources(verification.evidence_sources)}
                </div>
                <div class="verification-item full">
                  <span class="label">근거 요약</span><br>
                  ${escapeHtml(userFacingReportText(verification.evidence_summary, "-"))}
                </div>
                <div class="verification-item full">
                  <span class="label">부족한 맥락</span><br>
              ${escapeHtml(buildSafeMissingContext(result))}
                </div>
                <div class="verification-item">
                  <span class="label">마지막 확인 시간</span><br>
                  ${escapeHtml(verification.last_checked_at || "-")}
                </div>
              </div>
            </section>
            <div class="source-link">
    ...
    function buildReportTextLegacy() {
      const context = currentReportContext || {};
      const results = Array.isArray(context.results) ? context.results : [];
      const metrics = computeMetrics(results);
      const aggregate = aggregateEvidenceSummaries(results);
      ...
      lines.push("정책 AI 분석 리포트");
      lines.push("=".repeat(28));
      lines.push(`검색어: ${plain(context.query)}`);
      lines.push(`뉴스 개수: ${plain(context.maxNews || results.length, 0)}`);
      lines.push(`분석 시간: ${plain(context.analyzedAt)}`);
      lines.push(`최고 경고 단계: ${metrics.highest === "-" ? "-" : formatAlert(metrics.highest)}`);
      lines.push(`평균 신뢰도: ${plain(metrics.averageConfidence, 0)}`);
      lines.push(`고영향 뉴스 수: ${plain(metrics.highImpactCount, 0)}`);
      lines.push(`전체 evidence strength: ${formatEvidenceCounts(aggregate.strength)}`);
      lines.push(`전체 evidence quality: ${formatEvidenceCounts(aggregate.quality)}, avg ${plain(aggregate.quality.average_evidence_quality_score, 0)}`);
      lines.push("");
      ...
      results.forEach((result, index) => {
        ...
        const verification = result.verification_card || result || {};
        const evidence = getEvidenceSummaryForReport(result);
        const topSource = getTopSourceForReport(result);
        const reliability = verification.source_reliability_summary || {};
        const recommendation = reliability.official_mismatch
          ? "추가 공식 출처 확인 필요"
          : formatRecommendation(decision.action_recommendation);

        lines.push(`[${index + 1}] ${plain(result.title, "제목 없음")}`);
        lines.push("-".repeat(28));
        lines.push(`원문: ${plain(result.original_url)}`);
        lines.push(`주제: ${plain(result.topic)}`);
        lines.push(`경고 단계: ${formatAlert(String(decision.policy_alert_level || "WATCH").toUpperCase())}`);
        lines.push(`신뢰도 점수: ${plain(confidence.policy_confidence_score)}`);
        lines.push(`검증 강도: ${formatLevel(confidence.verification_strength)}`);
        lines.push(`영향도: ${formatLevel(impact.impact_level)}`);
        lines.push(`위험도: ${formatLevel(confidence.risk_level)}`);
        lines.push(`영향 방향: ${formatDirection(impact.impact_direction)}`);
        lines.push(`권장 조치: ${recommendation}`);
        lines.push(`시장 신호: ${formatSignal(decision.market_signal)}`);
        lines.push(`최고 신뢰 출처: ${plain(topSource.title, "정보 없음")}${topSource.url ? ` (${topSource.url})` : ""}`);
        if (reliability.official_mismatch) {
          lines.push(`공식 출처 상태: 공식 상세 근거 부족`);
          lines.push(`공식 출처 제외 이유: ${plain(reliability.official_mismatch_reasons, "정보 없음")}`);
        }
        lines.push(`evidence strength: ${formatEvidenceCounts(evidence.strength)}`);
        lines.push(`evidence quality: ${formatEvidenceCounts(evidence.quality)}, avg ${plain(evidence.quality.average_evidence_quality_score, 0)}`);
        lines.push(`AI 초안 판정: ${safeAiDraftVerdictForExport(result)}`);
        lines.push(`핵심 주장: ${sanitizePublicExportText(exportClaimText(result))}`);
        lines.push(`근거 요약: ${getSafeAiDraftEvidenceSummary(result)}`);
        lines.push(`부족한 맥락: ${getSafeAiDraftMissingContext(result)}`);
        lines.push(`마지막 확인 시간: ${plain(verification.last_checked_at)}`);
        lines.push("");
      });
    ...
    function serverReviewRenderDetail(task, decisions) {
      ...
      body.innerHTML = `
        <div class="server-review-detail-grid">
          ...
          <div><strong>원문 URL</strong>${task.url ? `<a href="${escapeHtml(task.url)}" target="_blank" rel="noopener noreferrer">${escapeHtml(task.url)}</a>` : "(없음)"}</div>

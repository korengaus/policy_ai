    ...
    // REAL settle measured against production was ~2.6s on a cold Render
    // instance — a shorter timer would fire spuriously on a cold start and
    // reintroduce the very double band paint this exists to prevent.
    // DELIBERATELY NOT A CANCELLATION: if the pick lands after expiry it still
    // upgrades the band (band-only, never the grid), so this timer changes
    // WHEN the band paints and never WHICH card is the hero.
    const HERO_BAND_SETTLE_TIMEOUT_MS = 3000;
    ...
    async function loadSpreadAnnotations() {
      ...
      for (const section of placeholders) {
        ...
        try {
          ...
          // NO new sentence pattern: 기사 {N}건 is the shipped form
          // web/claim.html:392 uses for the SAME quantity (cluster.member_count),
          // and (수집 기사 기준) is this block's own qualifier, taken from the
          // 확산 기간 line below. Circulation fact only — no chart, no ordering,
          // no emphasis, nothing implying importance.
          ...
          const sizeLine = Number.isFinite(memberSize) && memberSize >= outletCount
            ? `<div class="spread-size-line">기사 ${escapeHtml(memberSize)}건 (수집 기사 기준)</div>`
            : "";

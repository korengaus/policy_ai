    ...
    function renderResults(results, focusIndex = selectedResultIndex) {
      ...
      resultsEl.innerHTML = displayResults.map((result) => {
        ...
        const verifyNote = cleanConceptKeysForDisplay(contentLead
    ...
    if (requestedResultId) {
      // HISTORY-BACK baseline guard: a deep-link detail is the tab's current entry,
      // so pushing one detail entry on top leaves the load entry below as a "home
      // base" to land on — BACK clears the detail and shows the feed instead of
      // exiting to the previous site. (scrollY 0: a deep link loads at the top.)

...
- **M11.7c narrowing review:** **Intentionally broad** — narrowing reviewed and rejected. The try-body fans out across `_fetch_html_candidates` (requests family + potential `UnicodeDecodeError`), `_extract_candidate_text` (trafilatura + BeautifulSoup with undocumented exception surfaces — trafilatura usually returns `None` on failure but can raise lxml-level errors on malformed HTML; BS4 can raise `AttributeError` / `RecursionError` on pathological DOMs), and several string-op helpers. Narrowing to any tuple risks silently leaking exceptions on edge-case article HTML. Static + runtime pinned by `tests/test_m11_7c_exception_narrowing.py` (`BroadExceptHandlersIntentionalPin::test_site_2_broad_with_marker` + `Site2CatchesHttpFailurePin`).
...
- **M11.7c narrowing review:** **Intentionally broad — DECISIVE technical finding.** The `googlenewsdecoder` library's `decoderv2.py` raises bare `Exception("Failed to fetch data from Google.")`, `Exception("Header not found in response: ...")`, and `Exception("Footer not found in response.")` directly — these are *not* subclasses of any narrower type. Narrowing the Site 3a catch to ANY specific exception tuple would silently fail to catch these library-raised errors and break the "decoder failed → return original URL" fallback. Broad `except Exception` is also forward-compatible with future library bumps that might change exception classes. Static + runtime pinned by `tests/test_m11_7c_exception_narrowing.py` (`BroadExceptHandlersIntentionalPin::test_site_3a_broad_with_marker` + `Site3aCatchesLibraryBareExceptionPin` + `Site3aCatchesRequestsTimeoutPin`). The bare-Exception runtime pin is the most important guardrail in the M11.7c suite — it will fail loudly if a future PR narrows this handler.
...
- **M11.7c narrowing review:** **Intentionally broad** — narrowing reviewed and rejected. `extract_links_for_site` dispatches to per-site parsers (FSS / FSC / IBK / MOLIT / Gov24 / BOK / Assembly) whose failure modes are unbounded — BS4 `AttributeError` on layout changes, `KeyError` from internal mapping tables, `IndexError` from list ops, `ValueError` from `urljoin` on malformed hrefs. The contract is "a broken site-specific parser MUST NOT block the generic_fallback parser that runs below" — narrowing here would defeat that guarantee. Static + runtime pinned by `tests/test_m11_7c_exception_narrowing.py` (`BroadExceptHandlersIntentionalPin::test_site_5b_broad_with_marker` + `Site5bCatchesBs4AttributeErrorPin` — the latter confirms a simulated BS4 `AttributeError` from a per-site parser correctly falls through to `extract_official_result_links`).
...
- **M11.7c narrowing review:** **Intentionally broad — narrowing deferred pending production-log audit.** The try-body fans out across `_request_url` (RequestException family), `raise_for_status` (HTTPError), `_response_text` (UnicodeDecodeError), `_extract_html_text` (BS4 errors), and `_extract_candidate_links` (which falls through to `extract_official_result_links` OUTSIDE Site 5b's inner try → BS4 / `urljoin` errors can surface at this level). Narrowing to `RequestException` only would propagate BS4 / scoring errors UP to the outer wrapper (Site 5e), mis-classifying per-attempt parse failures as outer-wrapper failures and degrading the operational signal. The M11.7a-2 `exception_type` extra now collects the production data needed for a future evidence-based narrowing (see "Re-evaluation trigger" below). Static + runtime pinned by `tests/test_m11_7c_exception_narrowing.py` (`BroadExceptHandlersIntentionalPin::test_site_5c_broad_with_marker` + `Site5cCatchesHttpFailurePin`).
...
- **M11.7c narrowing review:** **Intentionally broad — narrowing deferred pending production-log audit.** The try-body fans out across `_request_url` (RequestException family + `MissingSchema` / `InvalidURL` on malformed candidate URLs), `raise_for_status` (HTTPError), `_extract_document_content` (BS4 / trafilatura), `_apply_candidate_title_fallback` / `_document_quality_exclusion_reason` (string ops), and `score_document_relevance` (KeyError / TypeError on malformed scoring inputs). Narrowing to `RequestException` only would mis-classify scoring / parsing errors as outer-wrapper failures. Same re-evaluation policy as Site 5c. Static + runtime pinned by `tests/test_m11_7c_exception_narrowing.py` (`BroadExceptHandlersIntentionalPin::test_site_5d_broad_with_marker` + `Site5dCatchesHttpErrorPin`).
...
| **M11.7c — Narrowing review of all 5 remaining sites** | 2, 3a, 5b, 5c, 5d | Reviewed each broad `except Exception` for narrowing feasibility. All 5 intentionally left broad with documented inline rationale + static + runtime pins. Decisive finding at Site 3a: `googlenewsdecoder` raises bare `Exception("...")`, so narrowing would silently leak library errors. | LOW | Small | ✅ Done (narrowing intentionally deferred) |
...
| **M11.7c-followup (future, gated on production data)** | 5c, 5d only | Re-evaluate narrowing for the per-attempt and per-candidate loops based on M11.7a-2's `exception_type` extras after 2-4 weeks of Render logs. Drop if data shows the broad catch only ever swallows RequestException-family + AttributeError. | LOW-MEDIUM | Small | Open (data-gated) |

**Re-evaluation trigger:** After 2-4 weeks of M11.7a-2 production data, operators should grep Render JSON logs for the per-site `exception_type` extras to determine if any of the 5 sites have a sufficiently narrow real-world exception distribution to justify a future narrowing pass. The relevant grep targets are:

- `article_extractor.fetch_failed.exception_type` (Site 2)
- `news_collector` records with the Korean `원문 URL 변환 실패` substring → `exception_type` extra (Site 3a)
- `official_crawler.site_specific_parser_failed.exception_type` (Site 5b)
- `official_crawler.attempt_failed.exception_type` (Site 5c)
- `official_crawler.candidate_evaluation_failed.exception_type` (Site 5d)

If `exception_type` is dominated by a small handful of types (typically `ConnectionError`, `Timeout`, `HTTPError`, plus the occasional `AttributeError` from BS4), a future M11.7c-followup PR could narrow at that site. If the distribution is long-tailed, broad `except Exception` remains correct.

**Resolution summary (as of M11.7c):** All 9 audit cites from §1.5 #8 are fully resolved. Sites 1, 4, 5a, 5e were closed by M11.7a / M11.7b / M11.5c. Sites 2, 3a, 5b, 5c, 5d had structured logging added in M11.7a-2 and then had narrowing reviewed in M11.7c — the review concluded all 5 should remain broad with documented inline rationale, plus static + runtime pins (`tests/test_m11_7c_exception_narrowing.py`, 11 tests) guarding against future accidental narrowing. Only M11.7d (Category 3 return-value tagging on Site 2) remains open as future work, gated on a separate verdict-path design review.
